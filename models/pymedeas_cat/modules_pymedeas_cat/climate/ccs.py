...
@component.add(
    ...
    comp_subtype="Normal",
    depends_on={
        "co2_captured_sector_tech_ccs_policy": 1,
...
def co2_captured_sector_ccs():
    ...
    return np.minimum(
        sum(
            co2_captured_sector_tech_ccs_policy().rename({"CCS tech": "CCS tech!"}),
            dim=["CCS tech!"],
        ),
...
@component.add(
    name="CO2 captured sector tech CCS policy",
    units="GtCO2/year",
    subscripts=["SECTORS and HOUSEHOLDS", "CCS tech"],
...
def co2_captured_sector_tech_ccs_policy():
    """
