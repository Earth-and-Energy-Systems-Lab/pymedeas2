...
@component.add(
    name="DEBUG historic coal extraction",
    units="EJ/Year",
    comp_type="Lookup",
    comp_subtype="External",
    depends_on={
        "__external__": "_ext_lookup_debug_historic_coal_extraction",
        "__lookup__": "_ext_lookup_debug_historic_coal_extraction",
    },
)
def debug_historic_coal_extraction(x, final_subs=None):
    return _ext_lookup_debug_historic_coal_extraction(x, final_subs)


_ext_lookup_debug_historic_coal_extraction = ExtLookup(
    "../debugging.xlsx",
    "Catalonia",
    "years_extraction",
    ...
    _root,
    {},
    "_ext_lookup_debug_historic_coal_extraction",
)
...
def extraction_coal_cat():
    ...
    return if_then_else(
        ...
        lambda: if_then_else(
            ...
            lambda: np.minimum(ped_domestic_cat_coal_ej(), max_extraction_coal()),
            lambda: np.minimum(
...
def extraction_coal_without_ctl_ej():
    ...
    """
    return np.maximum(extraction_coal_cat() - extraction_coal_for_ctl(), 0)
...
_ext_constant_urr_coal = ExtConstant(
    "../energy.xlsx", "Catalonia", "URR_coal", {}, _root, {}, "_ext_constant_urr_coal"
)
