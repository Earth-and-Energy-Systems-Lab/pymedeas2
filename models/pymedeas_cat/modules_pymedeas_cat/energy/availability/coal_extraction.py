...
def abundance_coal_aut():
    """
    Real Name: abundance coal AUT
    Original Eqn: IF THEN ELSE((extraction coal EJ AUT+imports AUT coal from RoW EJ )>PED coal EJ, 1, 1-ZIDZ( (PED coal EJ-extraction coal EJ AUT -imports AUT coal from RoW EJ), PED coal EJ))
    Units: Dmnl
    Limits: (None, None)
    Type: component
    Subs: None
    ...
    """
    return if_then_else(
        (extraction_coal_ej_aut() + imports_aut_coal_from_row_ej()) > ped_coal_ej(),
        lambda: 1,
        lambda: 1
        - zidz(
            (ped_coal_ej() - extraction_coal_ej_aut() - imports_aut_coal_from_row_ej()),
            ped_coal_ej(),
        ),
    )
...
def cumulated_coal_extraction():
    ...
    Original Eqn: INTEG ( extraction coal EJ AUT, cumulated coal extraction to 1995)
    Units: EJ
    Limits: (None, None)
    Type: component
    Subs: None
...
def cumulated_coal_extraction_to_1995():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'Austria', 'cumulative_coal_extraction_until_1995')
    Units: EJ
    Limits: (None, None)
    Type: constant
    Subs: None
...
def extraction_coal_ej_aut():
    """
    Real Name: extraction coal EJ AUT
    Original Eqn: IF THEN ELSE(RURR coal<0,0, IF THEN ELSE(Time<2016 :OR: "unlimited NRE?"=1 :OR: "unlimited coal?"=1, PED domestic AUT coal EJ,MIN(PED domestic AUT coal EJ, max extraction coal EJ)))
    Units: EJ
    Limits: (None, None)
    Type: component
    Subs: None
...
def extraction_coal_mtoe():
    ...
    Original Eqn: extraction coal EJ AUT*MToe per EJ
    Units: MToe/Year
    Limits: (None, None)
    Type: component
    Subs: None
    ...
    Annual extraction of coal.
    """
    return extraction_coal_ej_aut() * mtoe_per_ej()


def extraction_coal_without_ctl_ej():
    """
    Real Name: extraction coal without CTL EJ
    Original Eqn: MAX(extraction coal EJ AUT-extraction coal for CTL EJ, 0)
    Units: EJ/Year
    Limits: (None, None)
    Type: component
    Subs: None
    ...
    """
    return np.maximum(extraction_coal_ej_aut() - extraction_coal_for_ctl_ej(), 0)
...
def rurr_coal():
    """
    Real Name: RURR coal
    Original Eqn: INTEG ( -extraction coal EJ AUT-Flow coal left in ground, URR coal-cumulated coal extraction to 1995)
    Units: EJ
    Limits: (None, None)
    Type: component
    Subs: None
...
def share_rurr_coal_to_leave_underground():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_aut.xlsx', 'BAU', 'share_RURR_coal_underground')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def start_policy_leave_in_ground_coal():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_aut.xlsx', 'BAU', 'start_policy_year_coal_underground')
    Units: Year
    Limits: (None, None)
    Type: constant
    Subs: None
...
def table_max_extraction_coal(x):
    ...
    Original Eqn: ( GET DIRECT LOOKUPS('../energy.xlsx', 'Austria', 'RURR_coal', 'max_extraction_coal'))
    Units: EJ/Year
    Limits: (None, None)
    Type: lookup
    Subs: None
...
def unlimited_coal():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_aut.xlsx', 'BAU', 'unlimited_coal')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def urr_coal_input():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'Austria', 'URR_coal')
    Units: EJ
    Limits: (None, None)
    Type: constant
    Subs: None
...
def year_scarcity_coal():
    ...
    Original Eqn: IF THEN ELSE(abundance coal AUT>0.95, 0, Time)
    Units: Year
    Limits: (None, None)
    Type: component
    Subs: None
...
_integ_rurr_coal = Integ(
    lambda: -extraction_coal_ej_aut() - flow_coal_left_in_ground(),
...
_ext_constant_urr_coal_input = ExtConstant(
    "../energy.xlsx", "Austria", "URR_coal", {}, _root, "_ext_constant_urr_coal_input"
)
