...
def self_consuption_energy_sector():
    """
    0.164216*Required FED by liquids Required FED by gases*0.0917106
    """
    value = xr.DataArray(
        np.nan,
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"required_fed_by_liquids": 1, "ped_liquids": 1},
)
def share_oil_for_fc_emissions_relevant():
    """
    1-ZIDZ("Non-energy use demand by final fuel"[liquids],PED liquids)-share oil for Elec emissions relevant-share oil for Heat emissions relevant
    """
    return zidz(required_fed_by_liquids(), ped_liquids())
