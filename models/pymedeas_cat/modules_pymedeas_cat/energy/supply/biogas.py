...
@component.add(
    name="historic share biogas for electricity",
    units="Dmnl",
    comp_type="Lookup",
    comp_subtype="External",
    depends_on={
        "__external__": "_ext_lookup_historic_share_biogas_for_electricity",
        "__lookup__": "_ext_lookup_historic_share_biogas_for_electricity",
    },
)
def historic_share_biogas_for_electricity(x, final_subs=None):
    return _ext_lookup_historic_share_biogas_for_electricity(x, final_subs)


_ext_lookup_historic_share_biogas_for_electricity = ExtLookup(
    "../energy.xlsx",
    "Catalonia",
    "year_waste_biogas",
    ...
    _root,
    {},
    "_ext_lookup_historic_share_biogas_for_electricity",
)


@component.add(
    name="historic share PES biogas for CHP",
    units="Dmnl",
    comp_type="Lookup",
    comp_subtype="External",
    depends_on={
        "__external__": "_ext_lookup_historic_share_pes_biogas_for_chp",
        "__lookup__": "_ext_lookup_historic_share_pes_biogas_for_chp",
    },
)
def historic_share_pes_biogas_for_chp(x, final_subs=None):
    """
    Share of PES biogas for CHP plants.
    """
    return _ext_lookup_historic_share_pes_biogas_for_chp(x, final_subs)


_ext_lookup_historic_share_pes_biogas_for_chp = ExtLookup(
    "../energy.xlsx",
    "Catalonia",
    "year_waste_biogas",
    ...
    _root,
    {},
    "_ext_lookup_historic_share_pes_biogas_for_chp",
)
...
@component.add(
    name="policy biogas for electricity",
    units="Dmnl",
    comp_type="Lookup",
    comp_subtype="External",
    depends_on={
        "__external__": "_ext_lookup_policy_biogas_for_electricity",
        "__lookup__": "_ext_lookup_policy_biogas_for_electricity",
    },
)
def policy_biogas_for_electricity(x, final_subs=None):
    return _ext_lookup_policy_biogas_for_electricity(x, final_subs)


_ext_lookup_policy_biogas_for_electricity = ExtLookup(
    "../../scenarios/scen_cat.xlsx",
    "NZP",
    "year_RES_power",
    "share_biogas_elec",
    ...
    _root,
    {},
    "_ext_lookup_policy_biogas_for_electricity",
)
...
@component.add(
    name="share PES biogas CHP vs electricity and CHP",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "time": 3,
        "historic_share_pes_biogas_for_chp": 2,
        "historic_share_biogas_for_electricity": 1,
    },
)
def share_pes_biogas_chp_vs_electricity_and_chp():
    return zidz(
        historic_share_pes_biogas_for_chp(time()),
        historic_share_pes_biogas_for_chp(time())
        + historic_share_biogas_for_electricity(time()),
    )
...
@component.add(
    ...
    depends_on={
        "time": 2,
        "end_hist_data": 1,
        "historic_share_pes_biogas_for_chp": 1,
        "share_pes_biogas_for_elec_plants": 1,
        "share_pes_biogas_chp_vs_electricity_and_chp": 1,
    },
)
def share_pes_biogas_for_chp():
    return if_then_else(
        time() < end_hist_data(),
        lambda: historic_share_pes_biogas_for_chp(time()),
        lambda: share_pes_biogas_chp_vs_electricity_and_chp()
        * share_pes_biogas_for_elec_plants(),
    )
...
@component.add(
    ...
    depends_on={
        "time": 5,
        "end_hist_data": 5,
        "historic_share_biogas_for_electricity": 3,
        "policy_biogas_for_electricity": 2,
        "start_year_p_growth_res_elec": 3,
    },
)
def share_pes_biogas_for_elec_plants():
    ...
    return if_then_else(
        time() < end_hist_data(),
        lambda: historic_share_biogas_for_electricity(time()),
        lambda: if_then_else(
            time() < start_year_p_growth_res_elec(),
            lambda: historic_share_biogas_for_electricity(end_hist_data())
            + (
                (
                    policy_biogas_for_electricity(start_year_p_growth_res_elec())
                    - historic_share_biogas_for_electricity(end_hist_data())
                )
                / (start_year_p_growth_res_elec() - end_hist_data())
            )
            * (time() - end_hist_data()),
            lambda: policy_biogas_for_electricity(time()),
        ),
    )
...
@component.add(
    ...
    comp_subtype="Normal",
    depends_on={
        "share_pes_biogas_for_heatcom_plants": 1,
        "share_pes_biogas_for_elec_plants": 1,
        "share_pes_biogas_for_chp": 1,
    },
)
def share_pes_biogas_tfc():
    ...
    return (
        1
        - share_pes_biogas_for_heatcom_plants()
        - share_pes_biogas_for_elec_plants()
