...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"agricultural_land": 1, "nvs_1_to_m": 1, "population": 1},
)
def agricultural_land_pc():
    """
    Agricultural land per cápita.
    """
    return zidz(agricultural_land() * nvs_1_to_m(), population())
...
def urban_land_rate():
    ...
    return (
        if_then_else(
            ...
            lambda: 0.000795116 * urban_land_density_variation()
            + 2.03205 * 1e-08 * pop_variation(),
        )
        * aux_reach_ag_land()
    )
