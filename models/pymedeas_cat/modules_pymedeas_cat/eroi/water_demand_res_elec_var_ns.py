"""
Module water_demand_res_elec_var_ns
...
def total_water_for_om_required_by_res_elec():
    ...
    return xrmerge(
        rearrange(
            sum(total_water_for_om_required_by_res_elec_per_techn(), dim=("RES elec",)),
            ["water"],
            {"water": ["blue water"]},
        ),
...
def water_for_om__res_elec():
    ...
    Original Eqn:
      0
      GET DIRECT CONSTANTS('../materials.xlsx', 'Global', 'water_for_om_res_elec*')
    Units: kg/MW
    Limits: (None, None)
    Type: constant
    Subs: ['RES elec', 'water0']
    ...
    return xrmerge(
        ...
        _ext_constant_water_for_om__res_elec(),
    )
...
def water_for_om_required_for_res_elec():
    ...
    Original Eqn: installed capacity RES elec TW[RES elec]*"water for O&M - RES elec"[RES elec,water0]*M per T/kg per Mt
    Units: Mt
    Limits: (None, None)
    Type: component
    Subs: ['RES elec', 'water0']
...
_ext_constant_water_for_om__res_elec = ExtConstant(
    "../materials.xlsx",
    "Global",
