"""
Module natural_gas_extraction_ean
Translated using PySD version 2.1.0
"""


def abundance_total_nat_gas_aut():
    """
    Real Name: "abundance total nat. gas AUT"
    Original Eqn: IF THEN ELSE("PED nat. gas EJ"<"PES nat. gas AUT", 1, 1-ZIDZ(("PED nat. gas EJ"-"PES nat. gas AUT"), "PED nat. gas EJ" ))
    Units: Dmnl
    Limits: (None, None)
    Type: component
    Subs: None
    ...
    """
    return if_then_else(
        ped_nat_gas_ej() < pes_nat_gas_aut_1(),
        lambda: 1,
        lambda: 1 - zidz((ped_nat_gas_ej() - pes_nat_gas_aut_1()), ped_nat_gas_ej()),
    )
...
def cumulated_conv_gas_extraction_to_1995():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'Austria', 'cumulative_conventional_gas_extraction_until_1995')
    Units: EJ
    Limits: (None, None)
    Type: constant
    Subs: None
...
def cumulated_unconv_gas_extraction_to_1995():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'Austria', 'cumulative_unconventional_gas_extraction_until_1995')
    Units: EJ
    Limits: (None, None)
    Type: constant
    Subs: None
...
def extraction_conv_gas_ej():
    ...
    Original Eqn: IF THEN ELSE(RURR conv gas<0, 0, IF THEN ELSE(Time<2016 :OR: "unlimited NRE?"=1 :OR: "unlimited gas?"=1, "PED domestic AUT conv. nat. gas EJ", MIN("PED domestic AUT conv. nat. gas EJ", max extraction conv gas EJ)))
    Units: EJ/Year
    Limits: (None, None)
    Type: component
    Subs: None
    ...
    return if_then_else(
        ...
        lambda: if_then_else(
            ...
            lambda: ped_domestic_aut_conv_nat_gas_ej(),
            lambda: np.minimum(
                ped_domestic_aut_conv_nat_gas_ej(), max_extraction_conv_gas_ej()
            ),
        ),
    )
...
def extraction_tot_agg_gas_ej():
    ...
    Original Eqn: IF THEN ELSE(RURR tot agg gas<0,0, IF THEN ELSE(Time<2016 :OR: "unlimited NRE?"=1 :OR: "unlimited gas?"=1, "PED domestic AUT total nat.gas EJ", MIN("PED domestic AUT total nat.gas EJ", max extraction tot agg gas EJ)))
    Units: EJ/Year
    Limits: (None, None)
    Type: component
    Subs: None
    ...
    return if_then_else(
        ...
        lambda: if_then_else(
            logical_or(time() < 2016, unlimited_nre() == 1, unlimited_gas() == 1),
            lambda: ped_domestic_aut_total_natgas_ej(),
            lambda: np.minimum(
                ped_domestic_aut_total_natgas_ej(), max_extraction_tot_agg_gas_ej()
            ),
        ),
    )
...
def historic_unconv_gas():
    ...
    Original Eqn: GET DIRECT DATA('../energy.xlsx', 'Austria', 'time_historic_data', 'historic_unconventional_gas_extraction')
    Units: EJ/Year
    Limits: (None, None)
...
def pec_conv_gas():
    ...
    Original Eqn: real extraction conv gas EJ+imports AUT conv gas from RoW EJ
    Units: EJ
    Limits: (None, None)
    Type: component
    Subs: None
    ...
    """
    return real_extraction_conv_gas_ej() + imports_aut_conv_gas_from_row_ej()


def pec_unconv_gas():
    """
    Real Name: PEC unconv gas
    Original Eqn: real extraction unconv gas EJ+imports AUT unconv gas from RoW EJ
    Units: EJ
    Limits: (None, None)
    Type: component
    Subs: None
    ...
    """
    return real_extraction_unconv_gas_ej() + imports_aut_unconv_gas_from_row_ej()
...
def pes_nat_gas_without_gtl():
    ...
    Original Eqn: "PES nat. gas AUT"-"PED nat. gas for GTL EJ"
    Units: EJ/Year
    Limits: (None, None)
    Type: component
    Subs: None
    ...
    """
    return pes_nat_gas_aut_1() - ped_nat_gas_for_gtl_ej()
...
def share_unconv_gas_vs_tot_agg_in_2050():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'Austria', 'share_unconv_vs_agg_gas_in_2050')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def table_max_extraction_agg_gas(x):
    ...
    Original Eqn: ( GET DIRECT LOOKUPS('../energy.xlsx', 'Austria', 'RURR_agg_gas', 'max_extraction_agg_gas'))
    Units: EJ/Year
    Limits: (None, None)
    Type: lookup
    Subs: None
...
def table_max_extraction_conv_gas(x):
    ...
    Original Eqn: ( GET DIRECT LOOKUPS('../energy.xlsx', 'Austria', 'RURR_conv_gas', 'max_extraction_conv_gas'))
    Units: EJ/Year
    Limits: (None, None)
    Type: lookup
    Subs: None
...
def table_max_extraction_unconv_gas(x):
    ...
    Original Eqn: ( GET DIRECT LOOKUPS('../energy.xlsx', 'Austria', 'RURR_unconv_gas', 'max_extraction_unconv_gas'))
    Units: EJ/Year
    Limits: (None, None)
    Type: lookup
    Subs: None
...
def urr_conv_gas_input():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'Austria', 'URR_conv_gas')
    Units: EJ
    Limits: (None, None)
    Type: constant
    Subs: None
...
def urr_total_gas_input():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'Austria', 'URR_agg_gas')
    Units: EJ
    Limits: (None, None)
    Type: constant
    Subs: None
...
def urr_unconv_gas_input():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'Austria', 'URR_unconv_gas')
    Units: EJ
    Limits: (None, None)
    Type: constant
    Subs: None
...
def year_scarcity_total_nat_gas():
    ...
    Original Eqn: IF THEN ELSE("abundance total nat. gas AUT">0.95, 0, Time)
    Units:
    Limits: (None, None)
    Type: component
    Subs: None
    ...
    Year when the parameter abundance falls below 0.95, i.e. year when
        scarcity starts.
    """
    return if_then_else(abundance_total_nat_gas_aut() > 0.95, lambda: 0, lambda: time())
