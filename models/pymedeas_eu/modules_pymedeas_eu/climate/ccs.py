...
def co2_captured_by_sector_energy_related():
    ...
    value.loc[["Mining quarrying and energy supply"]] = np.minimum(
        float(
            co2_policy_captured_sector_ccs().loc["Mining quarrying and energy supply"]
        )
        * float(
...
@component.add(
    ...
    units="TWh/year",
    subscripts=[np.str_("dac tech"), "dac final sources"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "dac_per_tech": 2,
...
@component.add(
    name="total energy demand sector CCS",
    units="TWh/year",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"ccs_energy_demand_sect": 1},
