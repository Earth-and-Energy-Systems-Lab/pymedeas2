...
def ccs_cp():
    ...
    """
    return xr.DataArray(
        1, {"CCS_tech": _subscript_dict["CCS_tech"]}, [np.str_("CCS_tech")]
    )
...
def ccs_energy_demand_sect():
    return (
        sum(
            ccs_energy_demand_sect_tech().rename({np.str_("CCS_tech"): "CCS_tech!"}),
            dim=["CCS_tech!"],
        )
...
def co2_captured_by_sector_energy_related():
    ...
    value = xr.DataArray(
        np.nan,
        {"SECTORS_and_HOUSEHOLDS": _subscript_dict["SECTORS_and_HOUSEHOLDS"]},
        [np.str_("SECTORS_and_HOUSEHOLDS")],
    )
    ...
    value.loc[["Mining_quarrying_and_energy_supply"]] = np.minimum(
        float(
            co2_policy_captured_sector_ccs().loc["Mining_quarrying_and_energy_supply"]
        )
        * float(
            share_ccs_energy_related(time()).loc["Mining_quarrying_and_energy_supply"]
        ),
        float(
...
def co2_emissions_households_and_sectors_fossil_fuels():
    ...
    return sum(
        ...
        .rename({np.str_("final_sources"): "matter_final_sources!"}),
...
def co2_policy_captured_sector_ccs():
    ...
    """
    return sum(
        co2_captured_sector_tech_ccs().rename({np.str_("CCS_tech"): "CCS_tech!"}),
        dim=["CCS_tech!"],
    ) * (1 - float(scarcity_final_fuels().loc["electricity"]))
...
@component.add(
    name="DAC_efficiency",
    units="TWh/GtCO2",
    subscripts=[np.str_("dac_tech"), np.str_("dac_final_sources")],
    comp_type="Constant",
    comp_subtype="External",
...
@component.add(
    ...
    units="TWh/year",
    subscripts=[np.str_("dac_final_sources"), np.str_("SECTORS_and_HOUSEHOLDS")],
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def dac_energy_consumption_by_sector_and_fuel():
    return dac_energy_demand_per_sector_and_fuel() * (
        ...
        .rename({np.str_("final_sources"): "dac_final_sources"})
    )
...
@component.add(
    ...
    units="TWh/year",
    subscripts=[np.str_("dac_tech"), "dac_final_sources"],
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "dac_per_tech": 2,
...
def dac_energy_demand():
    value = xr.DataArray(
        ...
        [np.str_("dac_tech"), "dac_final_sources"],
    )
...
def dac_energy_demand_per_sector_and_fuel():
    return (
        sum(
            dac_energy_demand().rename({np.str_("dac_tech"): "dac_tech!"}),
            dim=["dac_tech!"],
        )
...
def total_dac_co2_captured():
    return sum(
        dac_co2_captured().rename({np.str_("dac_tech"): "dac_tech!"}), dim=["dac_tech!"]
    )
...
def total_dac_energy_demand():
    return sum(
        dac_energy_demand_per_sector_and_fuel().rename(
            {
                np.str_("dac_final_sources"): "dac_final_sources!",
                np.str_("SECTORS_and_HOUSEHOLDS"): "SECTORS_and_HOUSEHOLDS!",
            }
        ),
