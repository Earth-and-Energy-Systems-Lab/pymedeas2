...
def ch4_emissions_coal():
    ...
    value.loc[["heat"]] = (
        mtch4_per_ej_coal() * pec_coal() * share_coal_for_heat_emissions_relevant()
    )
    value.loc[["liquids"]] = (
...
@component.add(
    ...
    comp_type="Auxiliary, Constant",
    comp_subtype="Normal",
    depends_on={"pes_peat_ej": 1, "mtch4_per_ej_peat": 1},
)
def ch4_emissions_peat():
    ...
    except_subs.loc[["solids"]] = False
    value.values[except_subs.values] = 0
    value.loc[["solids"]] = pes_peat_ej() * mtch4_per_ej_peat()
    return value
