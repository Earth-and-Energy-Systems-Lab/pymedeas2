...
def abundance_coal_eu():
    ...
    """
    return if_then_else(
        extraction_coal_eu() + imports_eu_coal_from_row_ej() > ped_coal_ej(),
        lambda: 1,
        lambda: 1
        - zidz(
            ped_coal_ej() - extraction_coal_eu() - imports_eu_coal_from_row_ej(),
            ped_coal_ej(),
        ),
    )
...
@component.add(
    name="extraction coal EU",
    units="EJ",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
        "time": 1,
        "ped_domestic_eu_coal_ej": 3,
...
def extraction_coal_eu():
    ...
    return if_then_else(
        ...
        lambda: if_then_else(
            ...
            lambda: np.minimum(ped_domestic_eu_coal_ej(), max_extraction_coal()),
            lambda: np.minimum(
                np.minimum(ped_domestic_eu_coal_ej(), max_extraction_coal()),
...
@component.add(
    name="extraction coal EU delayed",
    units="EJ/Year",
    comp_type="Stateful",
    comp_subtype="DelayFixed",
...
@component.add(
    name='"share to leave underground feasible?"',
    units="Dmnl",
    comp_type="Auxiliary",
    comp_subtype="Normal",
