...
@component.add(
    name='"abundance_total_nat._gas_EU"',
    units="Dmnl",
    comp_type="Auxiliary",
    comp_subtype="Normal",
...
def check_gases():
    ...
    """
    return zidz(
        float(ped_fs().loc["gases"]) - float(pes_fs().loc["gases"]),
        float(pes_fs().loc["gases"]),
    )
...
def delay_gas_to_leave_underground():
    """
    This function is used so that the amount of gas to be left underground is substracted from the (technological) RURR from the Start year to leave gas undeground onwards.
    """
...
def extraction_conv_gas_ej():
    ...
    return if_then_else(
        ...
        lambda: if_then_else(
            time() < 2016,
            lambda: float(ped_domestic_eu_conv_ff().loc["gases"]),
            lambda: np.minimum(
                float(ped_domestic_eu_conv_ff().loc["gases"]),
...
def extraction_tot_agg_gas_ej():
    ...
    return if_then_else(
        time() < 2016,
        lambda: float(ped_domestic_ff().loc["gases"]),
        lambda: if_then_else(
            activate_force_leaving_underground() == 0,
            lambda: np.minimum(
                float(ped_domestic_ff().loc["gases"]), max_extraction_tot_agg_gas()
            ),
            lambda: np.minimum(
                np.minimum(
                    float(ped_domestic_ff().loc["gases"]), max_extraction_tot_agg_gas()
                ),
                remaining_extractable_fossil_gas_with_left_underground() / nvs_1_year(),
            ),
        ),
    )
...
@component.add(
    name="imports_EU_conv_gas_from_RoW_EJ",
    units="EJ/year",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
@component.add(
    name="imports_EU_unconv_gas_from_RoW_EJ",
    units="EJ/year",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"real_extraction_conv_gas": 1, "imports_eu_conv_gas_from_row_ej": 1},
)
def pec_conv_gas():
    return real_extraction_conv_gas() + imports_eu_conv_gas_from_row_ej()
...
@component.add(
    name='"PES_nat._gas_EU"',
    units="EJ/year",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"real_extraction_conv_gas": 1, "real_extraction_unconv_gas_ej": 1},
)
def pes_nat_gas_eu():
    return real_extraction_conv_gas() + real_extraction_unconv_gas_ej()
...
@component.add(
    name="real_extraction_unconv_gas_EJ",
    units="EJ/year",
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def remaining_extractable_fossil_gas_with_left_underground():
    ...
    """
    return np.maximum(0, rurr_tot_agg_gas() - delay_gas_to_leave_underground())
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"real_extraction_conv_gas": 2, "real_extraction_unconv_gas_ej": 1},
)
def share_conv_vs_total_gas_extraction_eu():
    ...
    return zidz(
        real_extraction_conv_gas(),
        real_extraction_conv_gas() + real_extraction_unconv_gas_ej(),
    )
...
def total_agg_fossil_gas_to_leave_underground():
    """
    Total aggregated gas to be left underground due to the application of policies that leave gas underground.
    """
    return (
