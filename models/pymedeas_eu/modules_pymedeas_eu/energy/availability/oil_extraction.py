...
def demand_conv_oil_ej():
    ...
    """
    return float(np.maximum(ped_total_oil_ej() - extraction_unconv_oil(), 0))
        ...
        lambda: if_then_else(
            ...
            lambda: float(
                np.minimum(
                    ...
                    max_extraction_conv_oil_policy(),
                )
            ),
            lambda: max_extraction_conv_oil_technical(),
        ),
...
def remaining_extractable_conv_oil_with_left_underground():
    ...
    """
    return float(np.maximum(0, rurr_conv_oil() - delay_conv_oil_to_leave_underground()))
...
def remaining_extractable_unconv_oil_with_left_underground():
    return float(
        np.maximum(0, rurr_unconv_oil() - delay_unconv_oil_to_leave_underground())
    )
