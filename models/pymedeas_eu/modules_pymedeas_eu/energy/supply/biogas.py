...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"fes_elec_from_biogas": 1, "ej_per_twh": 1},
)
def fes_elec_from_biogas_twh():
    """
    TFES electricity from biogas.
    """
    return fes_elec_from_biogas() / ej_per_twh()
...
def new_pes_biogas():
    ...
    return if_then_else(
        time() < 2014,
        lambda: (
            historic_biogas_pes(integer(time() + time_step()))
