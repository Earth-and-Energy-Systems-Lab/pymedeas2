...
def adapt_growth_phs():
    ...
    Original Eqn: IF THEN ELSE(Time<2015, past PHS capacity growth, IF THEN ELSE(Time<2020, past PHS capacity growth+(P PHS growth-past PHS capacity growth)*(Time -2015)/5, P PHS growth)*(1+"\"abundance\" storage"))
    Units: 1/year
    Limits: (None, None)
    Type: component
    Subs: None
...
def cp_phs():
    """
    Real Name: Cp PHS
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'cp_phs')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def initial_instal_cap_phs():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'initial_installed_capacity_phs')
    Units: TW
    Limits: (None, None)
    Type: constant
    Subs: None
...
def max_potential_phs_twe():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'G24')
    Units: TWe
    Limits: (None, None)
    Type: constant
    Subs: None
...
def p_phs_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'G36')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def past_phs_capacity_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'historic_growth_phs_capacity')
    Units: 1/year
    Limits: (None, None)
    Type: constant
    Subs: None
...
def phs_overcapacity():
    ...
    Original Eqn: MAX(0, ZIDZ( (potential FE elec stored PHS TWh - real FE elec stored PHS TWh), potential FE elec stored PHS TWh))
    Units: Dmnl
    Limits: (None, None)
    Type: component
    Subs: None
...
def potential_fe_elec_stored_phs_twh():
    ...
    Potential electricity stored in pumped hydro storage plants. It does not
        add up to the electricity generation of other sources since this
        electricity has already been accounted for! (stored).
    """
...
def real_fe_elec_stored_phs_twh():
    ...
    Original Eqn: MIN(max potential PHS TWh, potential FE elec stored PHS TWh)
    Units: TWh
    Limits: (None, None)
    Type: component
    Subs: None
...
def replacement_capacity_phs():
    ...
    Original Eqn: IF THEN ELSE(Time<2015,0,replacement rate PHS*wear PHS*(1-PHS overcapacity))
    Units: TW
    Limits: (None, None)
    Type: component
    Subs: None
...
def replacement_rate_phs():
    ...
    Original Eqn: IF THEN ELSE(potential FE elec stored PHS TWh<max potential PHS TWh ,1,0.8)
    Units: Dmnl
    Limits: (None, None)
    Type: component
    Subs: None
...
def table_hist_capacity_phs(x):
    ...
    Original Eqn: ( GET DIRECT LOOKUPS('../energy.xlsx', 'World', 'time_historic_data', 'historic_installed_capacity_phs'))
    Units: TW/year
    Limits: (None, None)
    Type: lookup
    Subs: None
...
_ext_constant_p_phs_growth = ExtConstant(
    "../../scenarios/scen_w.xlsx", "BAU", "G36", {}, _root, "_ext_constant_p_phs_growth"
)
