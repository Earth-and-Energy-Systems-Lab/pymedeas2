...
@component.add(
    ...
    subscripts=[np.str_("sectors"), np.str_("final_sources")],
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
@component.add(
    ...
    units="Dmnl/year",
    subscripts=[np.str_("SECTORS_and_HOUSEHOLDS"), np.str_("final_sources")],
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
...
def increase_of_intensity_due_to_energy_a_technology_change_top_down():
    ...
    return sum(
        increase_of_intensity_due_to_energy_a_technology_eff().rename(
            {
                np.str_("final_sources1"): "final_sources",
...
def increase_of_intensity_due_to_energy_a_technology_eff():
    ...
    return if_then_else(
        ...
        .rename(
            {
                np.str_("SECTORS_and_HOUSEHOLDS"): "sectors",
                np.str_("final_sources"): "final_sources1",
                np.str_("final_sources1"): "final_sources",
            }
        )
