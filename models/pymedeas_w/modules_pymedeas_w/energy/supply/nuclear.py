...
def cp_nuclear_initial():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'cp_initial_nuclear')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def effects_shortage_uranium():
    ...
    Original Eqn: IF THEN ELSE(extraction uranium EJ=0, 0, IF THEN ELSE(abundance uranium>0.8, ((abundance uranium-0.8)*5)^2, 0))
    Units: Dmnl
    Limits: (None, None)
    Type: component
    Subs: None
...
def historic_nuclear_generation_twh(x):
    ...
    Original Eqn: ( GET DIRECT LOOKUPS('../energy.xlsx', 'World', 'time_historic_data', 'historic_nuclear_generation'))
    Units: TWh/year
    Limits: (None, None)
    Type: lookup
    Subs: None
...
def initial_gen_nuclear():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'initial_nuclear_generation')
    Units: TWh
    Limits: (None, None)
    Type: constant
    Subs: None
...
def min_cp_nuclear():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'minimum_cp_nuclear')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def new_required_capacity_nuclear():
    ...
    Original Eqn: MAX(0, IF THEN ELSE(Time<2014, 0, IF THEN ELSE(Demand Elec NRE TWh=0, 0, installed capacity nuclear TW *P nuclear elec gen )))*effects shortage uranium*Cp limit nuclear
    Units: TW
    Limits: (None, None)
    Type: component
    Subs: None
...
def nuclear_capacity_under_construction():
    ...
    Original Eqn: IF THEN ELSE(Time<2013, (Historic nuclear generation TWh(Time+1)-Historic nuclear generation TWh(Time)) *TWe per TWh/Cp nuclear, Planned nuclear capacity TW/time construction nuclear)
    Units: TW
    Limits: (None, None)
    Type: component
    Subs: None
...
def replacement_nuclear_capacity():
    ...
    Original Eqn: IF THEN ELSE(Time<2013,Nuclear capacity under construction, IF THEN ELSE(selection of nuclear scenario=2, 0, IF THEN ELSE(selection of nuclear scenario=4, 0, replacement rate nuclear*wear nuclear*(1-nuclear overcapacity))))*Cp limit nuclear
    Units: TW
    Limits: (None, None)
    Type: component
    Subs: None
...
def wear_nuclear():
    ...
    Original Eqn: IF THEN ELSE(Time<2012, 0, installed capacity nuclear TW/life time nuclear)
    Units: TW
    Limits: (None, None)
    Type: component
    Subs: None
