...
def efficiency_res_heat():
    ...
    """
    value = xr.DataArray(
        np.nan, {"RES_heat": _subscript_dict["RES_heat"]}, [np.str_("RES_heat")]
    )
    value.loc[["solar_heat"]] = 1 - losses_solar_for_heat()
...
@component.add(
    name="Efficiency_solar_panels_for_heat",
    units="Dmnl",
    comp_type="Constant",
    comp_subtype="External",
...
def pes_res_for_heatcom_by_techn():
    ...
    value.loc[["solar_heat"]] = (
        float(fe_real_generation_res_heatcom_ej().loc["solar_heat"])
        * efficiency_solar_panels_for_heat()
...
def potential_fes_tot_res_for_heatcom():
    ...
    """
    return sum(
        potential_fes_res_for_heatcom().rename({np.str_("RES_heat"): "RES_heat!"}),
        dim=["RES_heat!"],
    )
