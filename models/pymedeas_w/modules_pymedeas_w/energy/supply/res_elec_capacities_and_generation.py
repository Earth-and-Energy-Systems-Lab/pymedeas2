...
def adapt_growth_res_elec():
    ...
    Original Eqn:
      IF THEN ELSE(Time<2015, past RES elec capacity growth[hydro], IF THEN ELSE(Time<Start year P growth RES elec, past RES elec capacity growth[hydro]+(P RES elec growth[hydro]-past RES elec capacity growth[hydro])*(Time-2015)/(Start year P growth RES elec -2015), P RES elec growth[hydro]))
        .
        .
        .
      IF THEN ELSE(Time<2014, past RES elec capacity growth[CSP], IF THEN ELSE(Time<Start year P growth RES elec, past RES elec capacity growth[CSP]+(P RES elec growth[CSP] - past RES elec capacity growth[CSP])*(Time-2014)/(Start year P growth RES elec-2014),P RES elec growth [CSP]))
    Units: 1/year
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
    ...
    Annual growth per RES elec technology. Modeling of a soft transition from
        current historic annual growth to reach the policy-objective 5 years later.
    """
    return xrmerge(
        rearrange(
            if_then_else(
                ...
                lambda: if_then_else(
                    ...
                    * (time() - 2015)
                    / (start_year_p_growth_res_elec() - 2015),
        ...
        rearrange(
            if_then_else(
                ...
                lambda: if_then_else(
                    ...
                    * (time() - 2013)
                    / (start_year_p_growth_res_elec() - 2013),
        ...
        rearrange(
            if_then_else(
                ...
                lambda: if_then_else(
                    ...
                    * (time() - 2014)
                    / (start_year_p_growth_res_elec() - 2014),
...
def cp_res_elec():
    ...
    Original Eqn: MAX(min Cp baseload RES[RES elec], "Cp-ini RES elec"[RES elec]*Cp exogenous RES elec reduction[RES elec])
    Units: Dmnl
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
...
def cpini_res_elec():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'cp_initial_res_elec*')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: ['RES elec']
...
def historic_new_required_capacity_res_elec():
    ...
    Original Eqn: table hist capacity RES elec[RES elec](Time+"total time plan+constr RES elec"[RES elec]+1)-table hist capacity RES elec[RES elec](Time+"total time plan+constr RES elec"[RES elec])
    Units: TW
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
...
def initial_instal_cap_res_elec():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'initial_installed_capacity_res_for_electricity*')
    Units: TW
    Limits: (None, None)
    Type: constant
    Subs: ['RES elec']
...
def min_cp_baseload_res():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'minimum_cp_baseload_res*')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: ['RES elec']
...
def new_required_capacity_res_elec():
    ...
    Original Eqn: IF THEN ELSE(Time<(2014-"total time plan+constr RES elec"[RES elec]), Historic new required capacity RES elec[RES elec],installed capacity RES elec TW[RES elec]*adapt growth RES elec after allocation[RES elec]*remaining potential constraint on new RES elec capacity [RES elec]*abundance RES elec2)
    Units: TW
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
...
def new_res_elec_capacity_under_planning():
    ...
    Original Eqn: MAX(0, required capacity RES elec TW[RES elec]/time planification RES elec[RES elec])
    Units: TW/year
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
    ...
    """
    return np.maximum(
        0, required_capacity_res_elec_tw() / time_planification_res_elec()
    )
...
def p_csp_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'p_CSP_growth')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def p_geot_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'p_geot_elect_growth')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def p_hydro_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'p_hydro_growth')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def p_oceanic_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'p_oceanic_growth_2020')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def p_solar_pv_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'p_solar_pv_growth')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def p_solid_bioeelec_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'p_solid_bioe_elect_growth')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def p_wind_offshore_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'p_wind_offshore_growth')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def p_wind_onshore_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'p_onshore_wind_growth')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def past_res_elec_capacity_growth():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'historic_growth_res_for_electricity*')
    Units: 1/year
    Limits: (None, None)
    Type: constant
    Subs: ['RES elec']
...
def potential_res_elec_after_intermitt_twh():
    ...
    Original Eqn: max potential RES elec TWe[RES elec]*Cp baseload reduction[RES elec] /TWe per TWh
    Units: TWh
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
...
def real_generation_res_elec_twh():
    ...
    Original Eqn: potential generation RES elec TWh[RES elec]*(1-RES elec tot overcapacity )
    Units: TWh
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
...
def replacement_capacity_res_elec():
    ...
    Original Eqn: IF THEN ELSE(Time<2015,0,replacement rate RES elec[RES elec]*wear RES elec[RES elec]*(1-RES elec tot overcapacity ))*constraint elec storage availability[RES elec]
    Units: TW/year
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
...
def res_elec_capacity_under_construction_tw():
    ...
    Original Eqn: RES elec planned capacity TW[RES elec]/time construction RES elec[RES elec]
    Units: TW/year
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
...
def start_year_p_growth_res_elec():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../../scenarios/scen_w.xlsx', 'BAU', 'start_year_P_growth_RES_elec')
    Units: year
    Limits: (None, None)
    Type: constant
    Subs: None
...
def table_hist_capacity_res_elec(x):
    ...
    Original Eqn: ( GET DIRECT LOOKUPS('../energy.xlsx', 'World', 'time_historic_data', 'historic_installed_capacity_res_for_electricity'))
    Units: TW/year
    Limits: (None, None)
    Type: lookup
    Subs: ['RES elec']
