...
def adapt_growth_res_for_heatnc():
    ...
    Original Eqn: IF THEN ELSE(Time<2014, "past RES growth for heat-nc"[RES heat], IF THEN ELSE(Time<Start year P growth RES heat , "past RES growth for heat-nc" [RES heat]+(P RES for heat[RES heat]-"past RES growth for heat-nc"[RES heat])*(Time-2014 )/(Start year P growth RES heat-2014), P RES for heat[RES heat]))
    Units: 1/year
    Limits: (None, None)
    Type: component
    Subs: ['RES heat']
...
def cp_res_for_heat():
    ...
    Original Eqn: "Cp-ini RES for heat"[RES heat]
    Units: Dmnl
    Limits: (None, None)
    Type: component
    Subs: ['RES heat']
...
def cpini_res_for_heat():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'cp_initial_res_heat*')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: ['RES heat']
...
def historic_res_capacity_for_heatnc(x):
    ...
    Original Eqn: ( GET DIRECT LOOKUPS('../energy.xlsx', 'World', 'time_historic_data', 'historic_res_capacity_for_heat_non_commercial'))
    Units: TW
    Limits: (None, None)
    Type: lookup
    Subs: ['RES heat']
...
def initial_value_res_for_heatnc():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'initial_res_capacity_for_heat_non_commercial*')
    Units: TW
    Limits: (None, None)
    Type: constant
    Subs: ['RES heat']
...
def new_res_capacity_for_heatnc_tw():
    ...
    Original Eqn: IF THEN ELSE(Time<2013, "Historic RES capacity for heat-nc"[RES heat](Time+1)-"Historic RES capacity for heat-nc"[RES heat ](Time), "adapt growth RES for heat-nc"[RES heat]*"installed capacity RES heat-nc TW"[RES heat]*remaining potential constraint on new RES heat capacity[RES heat])*"abundance RES heat-nc2"
    Units: TW/year
    Limits: (None, None)
    Type: component
    Subs: ['RES heat']
...
def past_res_growth_for_heatnc():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'historic_growth_res_for_heat_nc*')
    Units: 1/year
    Limits: (None, None)
    Type: constant
    Subs: ['RES heat']
...
def pes_res_for_heatnc_by_techn():
    ...
    Original Eqn:
      "FE real generation RES heat-nc EJ"[geot heat]/Efficiency RES heat[geot heat]
      "FE real generation RES heat-nc EJ"[solar heat]*Efficiency solar panels for heat/Efficiency RES heat[solar heat ]
...
def replacement_res_for_heatnc_tw():
    ...
    Original Eqn: "wear RES capacity for heat-nc TW"[RES heat]*"replacement RES for heat-nc"[RES heat]*(1-"RES heat-nc tot overcapacity" )
    Units: TW/year
    Limits: (None, None)
    Type: component
    Subs: ['RES heat']
