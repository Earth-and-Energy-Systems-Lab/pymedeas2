...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"total_fed_heatnc": 3, "fe_real_supply_res_for_heatnc_tot_ej": 1},
)
def abundance_res_heatnc():
    ...
    return if_then_else(
        ...
        lambda: 0,
        lambda: zidz(
            total_fed_heatnc() - fe_real_supply_res_for_heatnc_tot_ej(),
            total_fed_heatnc(),
        ),
    )
...
def fe_real_supply_res_for_heatnc_tot_ej():
    ...
    """
    return np.minimum(
        np.maximum(total_fed_heatnc(), 0), potential_fes_tot_res_for_heatnc_ej()
    )
...
def pes_res_for_heatnc_by_techn():
    ...
    value.loc[["geot heat"]] = float(
        fe_real_generation_res_heatnc().loc["geot heat"]
    ) / float(efficiency_res_heat().loc["geot heat"])
    value.loc[["solar heat"]] = (
        float(fe_real_generation_res_heatnc().loc["solar heat"])
