"""
Module energyavailabilityoil_extraction
...
def extraction_tot_agg_oil():
    ...
    return if_then_else(
        ...
        lambda: if_then_else(
            ...
            lambda: np.minimum(ped_total_oil_ej(), max_extraction_total_agg_oil()),
            lambda: np.minimum(
                np.minimum(ped_total_oil_ej(), max_extraction_total_agg_oil()),
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"real_extraction_conv_oil": 1, "real_extraction_unconv_oil_ej": 1},
)
def pes_oil_ej():
    """
    Total oil (conventional + unconventional) extraction.
    """
    return real_extraction_conv_oil() + real_extraction_unconv_oil_ej()
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"real_extraction_conv_oil": 1, "mbd_per_ejyear": 1},
)
def real_extraction_conv_oil_mbd():
    return real_extraction_conv_oil() * mbd_per_ejyear()
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"real_extraction_conv_oil": 2, "real_extraction_unconv_oil_ej": 1},
)
def share_conv_vs_total_oil_extraction():
    ...
    return zidz(
        ...
        real_extraction_conv_oil() + real_extraction_unconv_oil_ej(),
    )
...
def table_max_extraction_agg_oil(x, final_subs=None):
    """
    Data tables with maximum extraction of aggregated fossil oil due to technical constraints (Hubbert).
    """
...
def total_agg_fossil_oil_to_leave_underground():
    """
    Total aggregated fossil oil to be left underground due to the application of the policy to leave underground.
    """
    return (
...
def urr_tot_agg_oil_input():
    """
    Input of fossil oil URR (Ultimate Recoverable Resources).
    """
