"""
Module energydemandfe_intensity_households
...
def global_energy_intensity_h():
    ...
    """
    return sum(
        evol_final_energy_intensity_h().rename(
...
@component.add(
    ...
    units="EJ/(year*Tdollars)",
    subscripts=[np.str_("final sources1"), np.str_("final sources")],
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={
    ...
    return if_then_else(
        ...
        lambda: np.minimum(
            np.maximum(
                ...
                .loc["Households", :]
                .reset_coords(drop=True)
                .rename({np.str_("final sources"): "final sources1"}),
                0,
            ),
            1,
        ),
