...
def static_eroi_res_elec():
    ...
    Original Eqn:
      IF THEN ELSE(FEI over lifetime RES elec dispatch[hydro]=0,0,output elec over lifetime RES elec[hydro]/(FEI over lifetime RES elec dispatch[hydro]*quality of electricity))
        .
        .
        .
    ...
    Energy return on energy invested (over the full lifetime of the
        infrastructure) per RES technology for generating electricity. TODO
    """
    return xrmerge(
        rearrange(
            if_then_else(
...
def fei_over_lifetime_res_elec_var():
    ...
    Original Eqn: (CEDtot new cap RES elec var[RES elec]*(1+share energy requirements for decom RES elec[RES elec]+Grid correction factor RES elec [RES elec])+"CED O&M over lifetime RES elec var"[RES elec])*quality of electricity+output elec over lifetime RES elec[RES elec]*selfelectricity consumption RES elec[RES elec]
    Units: EJ
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
...
def fei_res_elec_var():
    ...
    Original Eqn: (CEDtot new cap RES elec var[RES elec]*(1+Grid correction factor RES elec[RES elec])+CED decom RES elec capacity [RES elec]+"CEDtot O&M RES elec var"[RES elec])*quality of electricity+real generation RES elec EJ[RES elec]*selfelectricity consumption RES elec[RES elec]
    Units: EJ
    Limits: (None, None)
    Type: component
    Subs: ['RES elec']
