...
def adapt_growth_res_for_heatcom():
    ...
    Original Eqn: IF THEN ELSE(Time<2014, "past RES growth for heat-com"[RES heat], IF THEN ELSE(Time<Start year P growth RES heat , "past RES growth for heat-com" [RES heat]+ (P RES for heat[RES heat]-"past RES growth for heat-com"[RES heat])*(Time-2014)/(Start year P growth RES heat-2014), P RES for heat [RES heat]))
    Units: 1/year
    Limits: (None, None)
    Type: component
    Subs: ['RES heat']
...
def efficiency_res_heat():
    ...
    Original Eqn:
      Efficiency solar panels for heat*Losses solar for heat
...
def efficiency_solar_panels_for_heat():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'efficiency_solar_panels_for_heat')
    Units: Dmnl
    Limits: (None, None)
    Type: constant
    Subs: None
...
def historic_res_capacity_for_heatcom(x):
    ...
    Original Eqn: ( GET DIRECT LOOKUPS('../energy.xlsx', 'World', 'time_historic_data', 'historic_res_capacity_for_heat_commercial'))
    Units: TW
    Limits: (None, None)
    Type: lookup
    Subs: ['RES heat']
...
def initial_value_res_for_heatcom():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'initial_res_capacity_for_heat_commercial*')
    Units: TW
    Limits: (None, None)
    Type: constant
    Subs: ['RES heat']
...
def new_res_capacity_for_heatcom_tw():
    ...
    Original Eqn: IF THEN ELSE(Time<2013, "Historic RES capacity for heat-com"[RES heat](Time+1)-"Historic RES capacity for heat-com"[RES heat](Time), "adapt growth RES for heat-com"[RES heat]*"installed capacity RES heat-com TW"[RES heat]*remaining potential constraint on new RES heat capacity [RES heat])*"abundance RES heat-com2"
    Units: TW/year
    Limits: (None, None)
    Type: component
    Subs: ['RES heat']
...
def past_res_growth_for_heatcom():
    ...
    Original Eqn: GET DIRECT CONSTANTS('../energy.xlsx', 'World', 'historic_growth_res_for_heat_com*')
    Units: 1/year
    Limits: (None, None)
    Type: constant
    Subs: ['RES heat']
...
def pes_res_for_heatcom_by_techn():
    ...
    Original Eqn:
      "FE real generation RES heat-com EJ"[geot heat]/Efficiency RES heat[geot heat]
      "FE real generation RES heat-com EJ"[solar heat]*Efficiency solar panels for heat/Efficiency RES heat[solar heat]
...
def remaining_potential_res_for_heat():
    ...
    Original Eqn: ZIDZ( (MAX(0, Max FE potential RES for heat[RES heat]-"potential FES RES for heat-com EJ"[RES heat]-"potential FES RES for heat-nc EJ"[RES heat])), Max FE potential RES for heat[RES heat] )
    Units: Dmnl
    Limits: (None, None)
    Type: component
    Subs: ['RES heat']
...
def replacement_res_for_heatcom_tw():
    ...
    Original Eqn: "wear RES capacity for heat-com TW"[RES heat]*"replacement RES for heat-com"[RES heat]*(1-"RES heat-com tot overcapacity" )
    Units: TW/year
    Limits: (None, None)
    Type: component
    Subs: ['RES heat']
