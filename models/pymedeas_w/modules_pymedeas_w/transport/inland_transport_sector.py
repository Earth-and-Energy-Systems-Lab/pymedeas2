...
@component.add(
    ...
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_p_bus_elec"},
)
def p_bus_elec():
    """
...
@component.add(
    ...
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_p_bus_gas"},
)
def p_bus_gas():
    """
...
@component.add(
    ...
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_p_bus_hyb"},
)
def p_bus_hyb():
    """
...
@component.add(
    ...
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_p_hv_gas"},
)
def p_hv_gas():
    """
...
@component.add(
    ...
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_p_hv_hyb"},
)
def p_hv_hyb():
    """
...
def p_inlandt():
    ...
    value.loc[["HV liq"]] = -p_hv_gas() - p_hv_hyb()
    value.loc[["HV hib"]] = p_hv_hyb()
    value.loc[["HV gas"]] = p_hv_gas()
    value.loc[["LV liq"]] = -p_lv_elec() - p_lv_hyb() - p_lv_gas()
    value.loc[["LV elec"]] = p_lv_elec()
    value.loc[["LV gas"]] = p_lv_gas()
    value.loc[["bus liq"]] = -p_bus_hyb() - p_bus_gas() - p_bus_elec()
    value.loc[["bus elec"]] = p_bus_elec()
    value.loc[["bus hib"]] = p_bus_hyb()
    value.loc[["bus gas"]] = p_bus_gas()
    value.loc[["train liq"]] = -p_train_elec()
    value.loc[["train elec"]] = p_train_elec()
    value.loc[["LV hib"]] = p_lv_hyb()
    return value
...
@component.add(
    ...
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_p_lv_elec"},
)
def p_lv_elec():
    """
...
@component.add(
    ...
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_p_lv_gas"},
)
def p_lv_gas():
    """
...
@component.add(
    ...
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_p_lv_hyb"},
)
def p_lv_hyb():
    """
...
@component.add(
    ...
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_p_train_elec"},
)
def p_train_elec():
    """
...
@component.add(
    ...
    other_deps={
        "_integ_percent_t_vehicles": {
            "initial": {"initial_percent_t_vehicles": 1},
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"real_total_output_by_sector": 1, "inland_transport_fraction": 1},
)
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"percent_t_vehicles": 32},
)
def shares_available_t():
    ...
    value.loc[["train liq"]] = 1 - float(percent_t_vehicles().loc["train elec"])
    value.loc[["train elec"]] = 1 - float(percent_t_vehicles().loc["train elec"])
    return value
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"energy_per_x_t": 3, "var_percent_t_vehicles": 3},
)
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"energy_per_x_t": 7, "var_percent_t_vehicles": 7},
)
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"adapt_var_inlandt": 18},
)
def var_percent_t_vehicles():
    ...
    value.loc[["HV liq"]] = -float(adapt_var_inlandt().loc["HV hib"]) - float(
        adapt_var_inlandt().loc["HV gas"]
    )
    value.loc[["HV hib"]] = float(adapt_var_inlandt().loc["HV hib"])
    value.loc[["HV gas"]] = float(adapt_var_inlandt().loc["HV gas"])
    value.loc[["LV liq"]] = (
        -float(adapt_var_inlandt().loc["LV hib"])
        - float(adapt_var_inlandt().loc["LV elec"])
        - float(adapt_var_inlandt().loc["LV gas"])
    )
    value.loc[["LV elec"]] = float(adapt_var_inlandt().loc["LV elec"])
    value.loc[["LV hib"]] = float(adapt_var_inlandt().loc["LV hib"])
    value.loc[["LV gas"]] = float(adapt_var_inlandt().loc["LV gas"])
    value.loc[["bus liq"]] = (
        -float(adapt_var_inlandt().loc["bus elec"])
        - float(adapt_var_inlandt().loc["bus hib"])
        - float(adapt_var_inlandt().loc["bus gas"])
    )
    value.loc[["bus hib"]] = float(adapt_var_inlandt().loc["bus hib"])
    value.loc[["bus gas"]] = float(adapt_var_inlandt().loc["bus gas"])
    value.loc[["train liq"]] = -float(adapt_var_inlandt().loc["train elec"])
    value.loc[["train elec"]] = float(adapt_var_inlandt().loc["train elec"])
