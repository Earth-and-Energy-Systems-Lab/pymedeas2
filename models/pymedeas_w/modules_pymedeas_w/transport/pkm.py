...
@component.add(
    name="efficiency liquids pkm",
    units="EJ/pkm",
    subscripts=["Transport Modes pkm"],
    comp_type="Constant",
    comp_subtype="External",
    depends_on={"__external__": "_ext_constant_efficiency_liquids_pkm"},
)
def efficiency_liquids_pkm():
    return _ext_constant_efficiency_liquids_pkm()


_ext_constant_efficiency_liquids_pkm = ExtConstant(
    "../transport.xlsx",
    "Global",
    "EJ_pkm_liquids",
    ...
    _root,
    {"Transport Modes pkm": _subscript_dict["Transport Modes pkm"]},
    "_ext_constant_efficiency_liquids_pkm",
)
...
@component.add(
    ...
    comp_subtype="Normal",
    depends_on={"energy_pkm": 4, "household_demand_total": 3, "time_step": 3},
)
def ei_households_transport():
    value = xr.DataArray(
        np.nan, {"final sources": _subscript_dict["final sources"]}, ["final sources"]
    )
    value.loc[["electricity"]] = float(energy_pkm().loc["elect", "Househ"]) / (
        household_demand_total() * 1e-06 * time_step()
    )
    value.loc[["gases"]] = float(energy_pkm().loc["gas", "Househ"]) / (
        household_demand_total() * 1e-06 * time_step()
    )
    value.loc[["liquids"]] = (
...
def pkm():
    ...
    return (
        if_then_else(
            ...
            lambda: (initial_pkmgdp() + pkmgdp_slope() * time()) * gdp(),
            lambda: hist_pkm_gdp(time()) * gdp(),
        )
        * time_step()
    )
