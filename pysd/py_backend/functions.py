...
class SampleIfTrue(Stateful):
    ...
    https://www.vensim.com/documentation/fn_sample_if_true.htm
    """

    def __init__(self, condition, sit_input, initial_value,
        ...
        ----------
        condition: function
          condition to retrieve the input value
        sit_input: function
          input value, returned if condition is True
        initial_value: function
          initial value
        py_name: str
          Python name to identify the object
    ...
    def __call__(self):
        # we profit the lazzy evaluation of if_then_else
        # working for scalars and xarray
...
class Model(Macro):
    ...
    def reload(self):
        """
        Reloads the model from the translated model file, so that
        all the parameters are back to their original value.
        """
        self.__init__(self.py_model_file, initialize=True,
                      missing_values=self.missing_values)

    def _default_return_columns(self, run=True):
        """
        ...
        ----------
        run: bool (optional)
            If True run cache objects are also returned.
        ...
        for key, value in self.components._namespace.items():
            if hasattr(self.components, value):
                ...
                if hasattr(func, 'type')\
                  and (run or getattr(func, 'type') == 'step'):
                    if value not in parsed_expr:
    ...
    def _integrate(self, time_steps, capture_elements, return_timestamps):
        ...
        #outputs = pd.DataFrame(None, index=return_timestamps, columns=capture_elements)
